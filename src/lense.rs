...
impl<O,V,G,S> Lense for L<O,V,G,S> where G:Fn(&O) -> V, S:Fn(&O,&V) -> O {
    type Object = O;
    type Value = V;
...
impl<'composite,OUTER,INNER,VALUE,L1,L2> Lense for Compose<'composite,OUTER,INNER,VALUE,L1,L2> 
where L1:'composite+Lense<Object=OUTER,Value=INNER>,
      L2:'composite+Lense<Object=INNER,Value=VALUE> {
    type Object = OUTER;
    type Value = VALUE;
    ...
    fn set(&self,obj:&Self::Object,val:&Self::Value) -> Self::Object {
        self.outer.set(obj,&(self.inner.set(&(self.outer.get(obj)),val)))
    }
}
...
mod tests {
    ...
    let turtle_position = lense(|turtle:&Turtle| { turtle.position },
                                 |turtle:&Turtle,pos:&Point| {
        Turtle { id:turtle.id, position: *pos}}
                                 );

    let point_x = lense(|point:&Point| { point.x },
                                 |point:&Point,x:&i32| {
        Point { x:*x,y:point.y}}
                                 );

    let turtle_point_x= Compose{outer:&turtle_position,inner:&point_x};

        let t1 = Turtle { id:1, position: Point{ x:0, y:0}};
        let t11 = Turtle { id:1, position: Point{ x:0, y:0}};
        let p = turtle_position.get(&t1);
        let t2 = turtle_position.set(&t11,&Point{x:9,y:9});

        let turtle_3 = Turtle { id:3, position: Point{ x:0, y:0}};
